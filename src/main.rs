...
struct Node {
    ...
    raft_group: Option<MerkleNode<MemStorage>>,
...
impl Node {
    ...
    fn create_raft_leader(
        ...
        let raft_group = Some(MerkleNode::new(&cfg, storage, &logger).unwrap());
        Node {
            raft_group,
            my_mailbox,
            mailboxes,
    ...
    fn initialize_raft_from_message(&mut self, msg: &Message, logger: &slog::Logger) {
        ...
        let storage = MemStorage::new();
        self.raft_group = Some(MerkleNode::new(&cfg, storage, &logger).unwrap());
    }
...
fn on_ready(
    raft_group: &mut MerkleNode<MemStorage>,
    ...
    let mut handle_committed_entries =
        |rn: &mut MerkleNode<MemStorage>, committed_entries: Vec<Entry>| {
